...
// import statc : 지정된 경로에 존재하는 static 구문을 모두 얻어와
// 클래스명.메서드명() 이 아닌 메서드명() 만 작성해도 호출 가능하게 함.
import static edu.kh.jdbc.common.JDBCTemplate.*;
import edu.kh.jdbc.model.dto.User;

// (Model 중 한) DAO (Data Access Object : 데이터 접근 객체)
...
public class UserDAO {
	...
	public List<User> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			// 4. ? (위치	홀더)에 알맞은 값 대입(없으면 패스)
			
			// 5. SQL(SELECT) 수행(executeQuery) 후 결과 반환(ResultSet) 받기
			rs = pstmt.executeQuery();
			
			// 6. 조회 결과(rs)를 1행ㅆ기 접근하여 컬럼 값 얻어오기
			// 몇 행이 조회될지 모른다 -> while
			// 무조건 1행만 조회된다  -> if
	 ...
	 * @throws Exception
	 */
	public User selectUser(Connection conn, int uno) throws Exception{
		User user = null;
		try {
			String sql = """
					SELECT USER_NO, USER_ID, USER_PW, USER_NAME, 
	 ...
	 * @throws Exception
	 */
	public int deleteUser(Connection conn, int uno) throws Exception {
	...
	public int idCheck(Connection conn, String userId) throws Exception {
		...
		try {
			...
			if(rs.next()) count = rs.getInt(1);  // 조회된 컬럼 순서번호를 이용해 컬럼값 얻어오기
		} finally {
			close(rs);
			close(pstmt);
		}
