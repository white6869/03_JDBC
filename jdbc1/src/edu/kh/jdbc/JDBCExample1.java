...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			// 2. DriberManager 객체를 이용해서 Connection 객체 생성하기
			// java.sql.DriverManager
			// - DB 연결 정보와 JDBC 드라이버를 이용해서
			//	원하는 DB와 연결할 수 있는 Connection 객체를 생성하는 객체
			
			// 2-1) Oracle JDBC Driver 객체를 메모리에 로드 ㅎ가ㅣ
			...
			String userName = "KH_BJH";		// 사용자 계정명
			...
			System.out.println(conn); // oracle.jdbc.driver.T4CConnection@4386f16
			...
			// -> sql이 SELECT 문일 때 결과를 ResultSet 객체 반환
			
			// 2) Statement.executUpdate(sql)
			...
			// 커서를 다음 행으로 이동 시킨 후
			// 이동된 해엥 값이 있으면 true, 없으면 false 반환
			// 맨 처음 호출 시 1행 부터 시작
			...
			while(rs.next()) {
				...
				// [java]				[db]
				// String			CHAR, VATCHAR2, NVARCHAR2
